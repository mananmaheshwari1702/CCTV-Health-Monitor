...
export function Dashboard() {
    ...
    const filteredData = React.useMemo(() => {
        ...
        // 2. Filter Tickets (based on device association or site)
        const filteredTickets = tickets.filter(t => {
            // Find device for this ticket to check site/type
            ...
            // Status filter only applies to devices, but if a device is filtered out, its tickets should ideally be hidden too?
            // Requirement says "Issue-wise filtering", commonly strictly for ticket status OR device status. 
            // Let's assume "Issue-wise" implies filtering tickets by status if selected, otherwise just standard scoping.
            return matchSite && matchType;
        });

        // 3. Re-calculate Stats
        const stats = {
    ...
    // Get only active/recent alerts for display, filtered by site if selected
                    ...
                    <div className="relative">
                        ...
                        {isConfigOpen && (
                            <div className="absolute right-0 top-full mt-2 w-64 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 z-50 p-4">
                                <h3 className="font-medium text-slate-900 dark:text-white mb-3">Dashboard Layout</h3>
                                <div className="space-y-3">
                                    <label className="flex items-center justify-between cursor-pointer">
                                        <span className="text-sm text-slate-600 dark:text-slate-300">Stats Overview</span>
                                        <input
                                            type="checkbox"
                                            className="accent-blue-600 w-4 h-4 rounded"
                                            checked={dashboardConfig.showStatsRow}
                                            onChange={(e) => updateDashboardConfig({ showStatsRow: e.target.checked })}
                                        />
                                    </label>
                                    <label className="flex items-center justify-between cursor-pointer">
                                        <span className="text-sm text-slate-600 dark:text-slate-300">Ticket Analytics</span>
                                        <input
                                            type="checkbox"
                                            className="accent-blue-600 w-4 h-4 rounded"
                                            checked={dashboardConfig.showTicketStats}
                                            onChange={(e) => updateDashboardConfig({ showTicketStats: e.target.checked })}
                                        />
                                    </label>
                                    <label className="flex items-center justify-between cursor-pointer">
                                        <span className="text-sm text-slate-600 dark:text-slate-300">Recent Alerts</span>
                                        <input
                                            type="checkbox"
                                            className="accent-blue-600 w-4 h-4 rounded"
                                            checked={dashboardConfig.showRecentAlerts}
                                            onChange={(e) => updateDashboardConfig({ showRecentAlerts: e.target.checked })}
                                        />
                                    </label>
                                    <label className="flex items-center justify-between cursor-pointer">
                                        <span className="text-sm text-slate-600 dark:text-slate-300">Latest Tickets</span>
                                        <input
                                            type="checkbox"
                                            className="accent-blue-600 w-4 h-4 rounded"
                                            checked={dashboardConfig.showLatestTickets}
                                            onChange={(e) => updateDashboardConfig({ showLatestTickets: e.target.checked })}
                                        />
                                    </label>
                                    <label className="flex items-center justify-between cursor-pointer">
                                        <span className="text-sm text-slate-600 dark:text-slate-300">Quick Actions</span>
                                        <input
                                            type="checkbox"
                                            className="accent-blue-600 w-4 h-4 rounded"
                                            checked={dashboardConfig.showQuickActions}
                                            onChange={(e) => updateDashboardConfig({ showQuickActions: e.target.checked })}
                                        />
                                    </label>
                                </div>
                            </div>
            ...
            {dashboardConfig.showStatsRow && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 md:gap-4">
                    ...
                    <StatsCard
                        ...
                        subtitle={`${stats.totalDevices > 0 ? Math.round((stats.onlineDevices / stats.totalDevices) * 100) : 0}% uptime`}
                        icon={Wifi}
                    ...
                    <StatsCard
                        ...
                        subtitle={`${stats.faultyDevices} faulty`}
                        icon={Heart}
                        iconColor="text-emerald-600"
                        iconBgColor="bg-emerald-50"
                    />
